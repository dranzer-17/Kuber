...
3. **A manager deactivates an employee who is mid-approval on 40 drafts.** Partially resolved: deactivation now requires picking a reassignment target for the employee's held campaigns/leads before it proceeds (§1.4 fix), and draft-approval authority follows campaign access (§2.8), so the new owner automatically gains approval rights on the transferred campaigns' drafts. Still open: nobody is actively *notified* there's a queue waiting — no notification system exists yet.
