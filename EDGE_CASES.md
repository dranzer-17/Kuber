...
9. **30 India leads are bulk-selected with two active India-territory employees.** Splits leads-first / least-loaded-first between the two, scoped to India-only load (§3.7) — *only* if the manager runs the bulk action with strategy `territory` or `round_robin`; picking `manual` with one named assignee dumps all 30 on that one person instead.
